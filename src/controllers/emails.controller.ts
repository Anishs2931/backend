...
export const sendEmails = async (_req: Request, res: Response): Promise<void> => {
    try {
...
export const verifyParticipant = async (req: Request, res: Response): Promise<void> => {
    try {
        const adminEmail = req.headers['x-admin-email'] as string;
        const adminPass = req.headers['x-admin-pass'] as string;

        if (!ADMIN_EMAILS.includes(adminEmail) || adminPass !== process.env.ADMIN_PASS) {
            res.status(403).json({
                success: false,
...
export const adminVerify = async (_req: Request, res: Response): Promise<void> => {
    try {
